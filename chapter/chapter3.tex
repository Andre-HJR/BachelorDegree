...
元数据是ring\_scheduler基本的调度单位，其包含了基本的任务信息，执行核心，地址空间信息，任务指针和相应的任务状态。
\begin{itemize}
...
如果需要使用ring\_scheduler，则在内核或用户的异步库中需要实现调度器的相应的接口约束，如下所示：
...
\end{equation}
    ...
    如\autoref{equation:c3schedulerreturn}所示，调度器的返回向量，应当记录了任务指针，对CPU使用权的让渡，任务的状态，和任务是否完成，其详细的描述如下：
\begin{itemize}
...
\autoref{code:kernelexecutor}是内核执行器的设计，在一个大循环中，不断从共享调度器中拿到任务，对任务进行poll操作，如果返回Ready，从共享调度器中删除该任务，如果返回Pending，将该任务设置为睡眠状态。如果共享调度器中所有任务都已完成，则退出系统。
\begin{figure}[h]
\begin{lstlisting}[caption=内核执行器的设计, label = code:kernelexecutor]
...
用户执行器的设计如\autoref{code:userexecutor} 用户执行器的设计和内核执行器存在一点区别，就是当拿到不在当前地址空间的任务的时候，不释放这个任务的内存，执行切换地址空间的系统调用。
\begin{figure}[h]
\begin{lstlisting}[caption=用户执行器的设计, label=code:userexecutor]
pub fn run_until_idle() {
    loop {
        let task = peek_task(); // 从共享调度器中拿出下一个任务的指针，不弹出
        match task {
...
\begin{figure}[htb]
    \figureCapSet
    \centering
    \includegraphics[scale=0.5]{figure/c3/starfishstructure.png}
