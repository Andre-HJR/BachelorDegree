...
waitpid 是一个立即返回的系统调用，它的返回值语义是：如果当前的进程不存在一个进程则返回-1；如果存在一个进程编号为pid的僵尸子进程，则正常回收并返回子进程的pid，并更新系统调用的退出码参数为 exit\_code 。这里还有一个 -2 的返回值，它的含义是子进程还没退出，通知用户库，当用户库看到是-2后，就进一步调用yield系统调用，让当前父进程进入等待状态。
